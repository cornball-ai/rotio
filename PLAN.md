# Plan: rotio as full pure-R OpenTimelineIO
...
rotio becomes a **complete pure-R reimplementation of OpenTimelineIO** — every
function `RcppOTIO` exports, including the hard ones (the composition coordinate
model and the edit algorithms). No compiled code; `Imports: jsonlite` only.
`RcppOTIO` (the Rcpp/libopentimelineio binding) is retained as the **oracle** for
parity tests, in `Suggests`.

"Are we missing functions?" stops being a question: the target is 100% of
RcppOTIO's surface. See `OTIO_PARITY.md` for the function-by-function matrix
(35 done, 88 to build, 14 rotio-only extras).
...
mirrors RcppOTIO's API. The target is **RcppOTIO API parity**, not a promise of
byte-for-byte interchange: once parity is proven, cornductor's low-level OTIO
construction/writing should become backend-swappable between RcppOTIO and pure-R
rotio with minimal adapter code. The functional builders stay as compatibility
...
its OTIO backend layer swappable; cornductor's purpose does not move into rotio.)
...
1. **Parent pointer is internal and never serialized.** `.parent` points to the
   ...
   (verified against RcppOTIO) — while tracks appended to that Stack have the Stack
...
3. **Single-parent invariant.** Attaching a child whose `.parent` is non-`NULL`
   **errors** ("child already has a parent"), matching RcppOTIO/OTIO (verified
   against RcppOTIO). Moving a child means `remove_child` first (or a documented
...
4. **`clone(x)` is deep.** Returns a deep copy of the subtree; the returned
   ...
   Verified: RcppOTIO resets the clone root's parent to `NULL`.
5. **JSON parse reconstructs parent pointers.** `from_json_string` / `_file`
   ...
   (byte-compatible with RcppOTIO, `.parent` stripped).
...
7. **Parity is judged externally, not by representation.** Equality/parity tests
   compare (a) normalized serialized OTIO JSON (round-tripped through RcppOTIO) and
   (b) selected behavioral queries (`range_in_parent`, `trimmed_range`,
   `find_clips`, `duration`, edit-algorithm results) against RcppOTIO on a fixture
   battery. Internal pointers and R representation are never compared.

## Phases (each validated against RcppOTIO before merge)
...
- **Phase 5 — edit algorithms.** `overwrite`, `insert`, `trim`, `slice`, `slip`,
  `slide`, `ripple`, `roll`, `fill`, `remove`. Each diffed against RcppOTIO on a
...
- `RcppOTIO` as oracle, `Suggests`-gated and `at_home()`/`requireNamespace`-guarded
  so a portable `R CMD check` (no RcppOTIO) still passes.
- For every ported function: build the same object in rotio and RcppOTIO, assert
  (a) the oracle round-trip matches —
  `RcppOTIO::to_json_string(RcppOTIO::from_json_string(rotio::to_json_string(x)))`
  equals `RcppOTIO::to_json_string(rotio_native)` (RcppOTIO cannot serialize a pure-R
  rotio object directly, so our JSON is parsed by RcppOTIO first) — and
...
- **cornductor**: stays the AI/project conductor. Full parity makes its
  low-level OTIO construction/writing layer backend-swappable between RcppOTIO and
  pure-R rotio (ideally minimal adapter code) once parity is proven — not a
  promise of literal sed-level replacement up front.
- **"rotiolite" naming** is retired; rotio is full pure-R OTIO. The Title/
