# Phase 5 edit algorithms: every case is validated against RcppOTIO (the oracle),
# which wraps the same OTIO C++ algorithm rotio ports.

if (requireNamespace("RcppOTIO", quietly = TRUE)) {

    # ---- builders (rotio + RcppOTIO), identical structure ----
    ...
    rclip <- function(n, s, d, avail = NULL) {
        ref <- RcppOTIO::ExternalReference(paste0(n, ".mov"))
        if (!is.null(avail)) {
            RcppOTIO::`available_range<-`(ref, RcppOTIO::TimeRange(RcppOTIO::RationalTime(avail[1], 24), RcppOTIO::RationalTime(avail[2], 24)))
        }
        RcppOTIO::Clip(n, ref, source_range = RcppOTIO::TimeRange(RcppOTIO::RationalTime(s, 24), RcppOTIO::RationalTime(d, 24)))
    }
    ngap <- function(d) Gap(RationalTime(d, 24))
    rgap <- function(d) RcppOTIO::Gap(RcppOTIO::RationalTime(d, 24))
    ...
    rtrack <- function(items) {
        t <- RcppOTIO::Track("V")
        for (it in items) RcppOTIO::append_child(t, it)
        t
    }
    ...
    rsnap <- function(t) lapply(RcppOTIO::children(t), function(c) {
        sr <- tryCatch(RcppOTIO::source_range(c), error = function(e) NULL)
    ...
    rRT <- function(v) RcppOTIO::RationalTime(v, 24)
    ...
    for (dv in c(0, 2, -2, 100, -100)) {
        ...
        RcppOTIO::slip(RcppOTIO::children(rt)[[1]], rRT(dv))
    ...
    for (din in c(0, 2, -3)) for (dout in c(0, 2, -3, 100)) {
        ...
        RcppOTIO::ripple(RcppOTIO::children(rt)[[1]], rRT(din), rRT(dout))
    ...
    for (dv in c(0, 2, -2, 100)) {
        ...
        RcppOTIO::slide(RcppOTIO::children(rt)[[2]], rRT(dv))
    ...
    for (din in c(0, 2, -2)) for (dout in c(0, 2, -2)) {
        ...
        RcppOTIO::trim(RcppOTIO::children(rt)[[2]], rRT(din), rRT(dout))
        ...
        RcppOTIO::trim(RcppOTIO::children(rg)[[2]], rRT(din), rRT(dout))
    ...
    for (din in c(0, 2, -2)) for (dout in c(0, 2, -2)) {
        ...
        RcppOTIO::roll(RcppOTIO::children(rt)[[2]], rRT(din), rRT(dout))
...
if (requireNamespace("RcppOTIO", quietly = TRUE)) {
    ...
    for (tv in c(3, 5, 10, 12, 0, 16)) {
        ...
        tryCatch(RcppOTIO::slice(rt, rRT(tv)), error = function(e) NULL)
    ...
    for (tv in c(2, 9, 14)) for (fl in c(TRUE, FALSE)) {
        ...
        RcppOTIO::remove(rt, rRT(tv), fill = fl)
    ...
    for (tv in c(4, 8, 0, 100)) {
        ...
        RcppOTIO::insert(rclip("X", 0, 5), rt, rRT(tv))
...
if (requireNamespace("RcppOTIO", quietly = TRUE)) {
    ...
    for (spec in list(c(3, 4), c(6, 8), c(0, 5), c(8, 8), c(0, 16), c(2, 10), c(20, 4), c(-5, 3))) {
        ...
        tryCatch(RcppOTIO::overwrite(rclip("X", 0, 100), rt, RcppOTIO::TimeRange(rRT(s), rRT(d))), error = function(e) NULL)
...
if (requireNamespace("RcppOTIO", quietly = TRUE)) {
    ...
    # triggers an OTIO 0.18.1 remove_child bug in RcppOTIO; rotio is correct there
    ...
    for (rp in c("Source", "Sequence")) {
        for (cd in c(6, 4)) {
            ...
            tryCatch(RcppOTIO::fill(rclip("X", 2, cd), rt, rRT(8), reference_point = rp), error = function(e) NULL)
    ...
    RcppOTIO::fill(rclip("X", 0, 4), rt, rRT(8), reference_point = "Fit")
    expect_equal(vapply(children(nt), function(c) class(c)[1], ""),
                 vapply(RcppOTIO::children(rt), function(c) class(c)[1], ""))   # Clip, Gap, Item, Clip
    ...
    # ---- overwrite over a 3-item span: parity for cases RcppOTIO gets right ----
    # (8,10) and (10,8) span an item to the composition end, which trips the OTIO
    # 0.18.1 remove_child-by-pointer bug in RcppOTIO; rotio is correct (asserted
    ...
    for (spec in list(c(6, 4), c(3, 8), c(5, 13), c(2, 16), c(0, 18))) {
        ...
        tryCatch(RcppOTIO::overwrite(rclip("X", 2, 100), rt, RcppOTIO::TimeRange(rRT(s), rRT(d))), error = function(e) NULL)
        expect_equal(nsnap(nt), rsnap(rt), info = paste("overwrite3", s, d))
    }
}

# Correct behaviour where OTIO 0.18.1 (and thus RcppOTIO) is buggy: remove_child is
# called with a pointer but only takes an int, so it deletes the wrong child.
# rotio removes the intended item; these assert the correct partition results.
mk_t <- function() {
    t <- Track("V")
